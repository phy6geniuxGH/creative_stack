class Object {
  ...
  float activateXwall, activateYwall ; 
  ...
  /*Fluid Drag Functions
   This one has an object dependence on the StaticFluid, where some of its
   parameters were taken, unlike the friction
   */
  ...
  boolean isFluid(StaticFluid L_) {   //Defines the boundaries of the pocket
    if (L_.square && pos.x > L_.x &&                        //This condition tells if the object
      ...
      pos.y < L_.y + L_.h) {                    //StaticFluid class since we declare it
      return true;                           // (x,y,w,h) in the Object class.
  ...
  void hardWallBCx(hardWalls HW_) {
    //vel.x *= -1;
    //vel.y *= -1;
    if(pos.y+20 > HW_.y && pos.y < HW_.y+HW_.h-20){
      vel.y *= -1;
    }
    if(pos.x+20 > HW_.x && pos.x < HW_.x+HW_.w-20){
      vel.x *= -1;
    }
  }
  void hardWallBCy() {
  ...
  boolean isHW(hardWalls HW_) {   //Defines the boundaries of the pocket
    if (HW_.square && pos.x > HW_.x &&                        //This condition tells if the object
      pos.x < HW_.x + HW_.w &&                     //is within the domain of the pocket.
      pos.y > HW_.y &&                          //you can access the properties of the 
      pos.y < HW_.y + HW_.h ) {           //hardWalls class since we declare it
      return true;                           // (x,y,w,h) in the Object class.
    } else if (HW_.circle && pos.x > HW_.x - abs((0.40*sqrt(pow(HW_.w, 2) + pow(HW_.h, 2)))*cos(atan(pos.y/pos.x))) &&                        //This condition tells if the object
      pos.x < HW_.x + abs((0.40*sqrt(pow(HW_.w, 2) + pow(HW_.h, 2)))*cos(atan(pos.y/pos.x))) &&                   //is within the domain of the pocket.
      pos.y > HW_.y - abs((0.40*sqrt(pow(HW_.w, 2) + pow(HW_.h, 2)))*sin(atan(pos.y/pos.x))) &&                          //you can access the properties of the 
      pos.y < HW_.y + abs((0.40*sqrt(pow(HW_.w, 2) + pow(HW_.h, 2)))*sin(atan(pos.y/pos.x)))) {                    //Pockets class since we declare it                                                                                   //we used the pythagorean theorem here!         
      return true;
    } else if (HW_.rectangle && pos.x > HW_.x &&                        //This condition tells if the object
      pos.x < HW_.x + 2*HW_.w &&                   //is within the domain of the pocket.
      pos.y > HW_.y &&                          //you can access the properties of the 
      pos.y < HW_.y + HW_.h) {                    //Pockets class since we declare it     //The rectangle shape has a 2:1 width-height ratio
      return true;
    } else { 
      return false;
    }
